...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld) payable ERC721("Doge Name Service", "DNS") {
    ...
    function getAllNames() public view returns (string[] memory) {
        console.log("Getting all domains from contract");
        string[] memory allnames = new string[](_tokenIds.current());
        for (uint256 i = 0; i < _tokenIds.current(); i++) {
            allnames[i] = domainNames[i];
            console.log("Domain name: %s with tokenId %d", allnames[i], i);
        }
        return allnames;
    }
    ...
    function register(string calldata name) public payable {
        ...
        _setTokenURI(newRecordId, "ipfs://QmZAGHMLRzVGDsSrjCepNpi6ofGf4LctHbpewvRhbVmSca");
        domains[name] = msg.sender;
        domainNames[newRecordId] = name;
        _tokenIds.increment();
    }
